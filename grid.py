...
class Grid:
    ...
    def _place_building(self, start_x: int, start_y: int, width: int, height: int) -> bool:
        ...
        for y in range(start_y, start_y + height):
            for x in range(start_x, start_x + width):
                if not self._is_valid_position(x, y) or self.grid[y][x] != 0:
                    return False
    ...
    def _place_road(self, x: int, y: int, width: int = 1) -> bool:
        ...
        for w in range(width):
            if not self._is_valid_position(x + w, y) or self.grid[y][x + w] not in [0, 'r']:
                return False
    ...
    def _is_road_connected(self, x: int, y: int) -> bool:
        ...
        for dx, dy in directions:
            ...
            if self._is_valid_position(new_x, new_y) and self.grid[new_y][new_x] == 'r':
                return True
        return False
    ...
    def _is_adjacent_to_road(self, start_x: int, start_y: int, width: int, height: int) -> bool:
        ...
        for x in range(start_x - 1, start_x + width + 1):
            if self._is_valid_position(x, start_y - 1) and self.grid[start_y - 1][x] == 'r':
                return True
            if self._is_valid_position(x, start_y + height) and self.grid[start_y + height][x] == 'r':
                return True
        
        for y in range(start_y - 1, start_y + height + 1):
            if self._is_valid_position(start_x - 1, y) and self.grid[y][start_x - 1] == 'r':
                return True
            if self._is_valid_position(start_x + width, y) and self.grid[y][start_x + width] == 'r':
                return True
            ...
            for w in range(road_width):
                if y + w < self.height:
                    for x in range(self.width):
                        if self.grid[y + w][x] != 'r':
                            self.grid[y + w][x] = 'r'
                            road_cells += 1
            y += random.randint(min_building_size, max_building_size+1)
            ...
            for w in range(road_width):
                if x + w < self.width:
                    for y in range(self.height):
                        if self.grid[y][x + w] != 'r':
                            self.grid[y][x + w] = 'r'
                            road_cells += 1
            x += random.randint(min_building_size, max_building_size+1)
